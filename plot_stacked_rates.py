def plotRates(df_ALL, df_RAIN, df_DRY, title, save_filename = None):
    dfs = [df_ALL, df_RAIN, df_DRY]
    fig, axs = plt.subplots(nrows=3, figsize=(18, 30))
...
#         axs[i].set_ylim(y_min, y_max)
        ...
        loss_legend = axs[i].legend(handles=loss_lines, loc='lower right', bbox_to_anchor = (0.9, 0.0), title='Loss Rates', frameon=False, fontsize=14)
        ...
        loss_legend.get_title().set_fontsize(12)
        loss_legend.get_title().set_ha('left') # title of legend left justified
        prod_legend.get_title().set_fontsize(12)
...
#plotRates(df_ALL, df_RAIN, df_DRY, ['All data', 'Rain days', 'Dry days'], save_filename = 'ReactionRates_pH6_adjustedRates')
